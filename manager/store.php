...
if (isset($_POST['searchBook'])) {
    ...
    if ($book_name != "" && isset($_POST['category']) && isset($_POST['series']) && isset($_POST['printery'])) {
        ...
        $sql = "SELECT count(book_id) as count 
            FROM book FROM book NATURAL JOIN printery NATURAL JOIN category NATURAL JOIN series
            WHERE printery_id = ? AND category_id = ? AND series_id = ? AND bquantity <> 0";
        $countBook = $pdo->prepare($sql);
        $countBook->bindValue(1, $printery, PDO::PARAM_INT);
...
<body class="w3-theme-l5">
    ...
    <div class="w3-row">
        ...
        <div class="w3-card-4 w3-margin w3-rest" style="background-color: white;">
            <h1 class="w3-center w3-padding">Book Store</h1>
            <div class="w3-row">
                            ...
                            <tbody>
                                <?php while ($rs = $book->fetch(PDO::FETCH_ASSOC)) { ?>
                                    <tr>
                                        ...
                                        <td class=""><?php echo $rs['sname'] ?></td>
                                        <td class="" style="width:25%; height: 50px;">
                                            <button type="button" data-id="<?php echo $rs['book_id'] ?>" class="btn btn-outline-info px-3 edit">
                                                <i class="fa fa-info" aria-hidden="true"></i></button>

                                            <button type="button" data-id="<?php echo $rs['book_id'] ?>" class="btn btn-outline-success px-3 show_modal">
                                            <i class="fa fa-plus-square-o" aria-hidden="true"></i></button>

                                            <button type="button" data-id="<?php echo $rs['book_id'] ?>" class="btn btn-outline-danger px-3 delete"><i class="fa fa-times" aria-hidden="true"></i></button>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
    ...
    <!-- quatity modal -->
    <div class="modal fade bd-example-modal-sm" id="qty_modal" role="dialog">
        <div class="w3-modal-content w3-animate-zoom w3-card-4" style="max-width:500px">
            ...
            <div class="w3-conatiner">
                <div class="w3-container w3-center">
                    ...
                    <input type="number" value="1" min="1" id="bquantity" name="bquantity" class="form-control mb-4" placeholder="">
                </div>
            </div>
            <div class="w3-container">
    ...
    <!-- edit modal -->
    <div class="modal fade bd-example-modal-lg" id="edit_book" role="dialog">
        <div class="w3-modal-content w3-animate-zoom w3-card-4" style="max-width:600px">
            <div class="modal-dialog modal-lg"></div>
            <div class="w3-conatiner w3-theme-d3">
                <p class="w3-xxxlarge w3-center w3-margin">Edit Book</p>
            </div>
            <br>
            <div class="w3-container">
                <form method="post" action="">
                    <label for="textInput">Book Name</label>
                    <input type="text" id="bname" name="bname" class="form-control mb-4">

                    <label for="textInput">Price</label>
                    <input type="number" min="1" id="bprice" name="bprice" class="form-control mb-4">

                    <label for="textInput">Series</label>
                    <select class="w3-select w3-border" name="series" id="series">
                        <?php $ser->execute();
                        while ($rs = $ser->fetch(PDO::FETCH_ASSOC)) { ?>
                            <option value="<?php echo $rs['series_id'] ?>"><?php echo $rs['sname'] ?></option>
                        <?php } ?>
                    </select><br><br>

                    <label for="textInput">Category</label>
                    <select class="w3-select w3-border" name="category" id="category">
                        <?php $cat->execute();
                        while ($rs = $cat->fetch(PDO::FETCH_ASSOC)) { ?>
                            <option value="<?php echo $rs['category_id'] ?>"><?php echo $rs['ctitle'] ?></option>
                        <?php } ?>
                    </select><br><br>
            ...
            <div class="w3-container">
                <div class="w3-container w3-center w3-margin ">
                    <button type="button" id="update_book" class="btn btn-success">Confirm</button>
                    <button type="button" id="close" class="btn btn-danger">Cancel</button>
                </div>
            </div>
        </div>
